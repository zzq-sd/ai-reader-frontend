...
async function sendMessage() {
  ...
  try {
    ...
    if (props.articleContent && props.articleContent.trim()) {
      // 截取文章内容的前2000个字符作为上下文
      const articleContext = props.articleContent.substring(0, 2000)
      contextualMessage = `基于以下文章内容回答问题：\n\n文章内容：${articleContext}\n\n问题：${currentInput}`
    } else if (props.articleId) {
      contextualMessage = `基于当前文章内容，${currentInput}`
    }
    
    // 调用AI API
    const response = await aiChatApi.chat(contextualMessage)

    let aiContent = ''
    if (response.response) {
      aiContent = response.response
    } else if (response.fallback) {
      aiContent = response.fallback
    } else if (response.error) {
      aiContent = `抱歉，${response.error}`
    } else {
      aiContent = '抱歉，我暂时无法回复您的问题。'
    }
    ...
    // 如果是总结类请求，触发保存事件
    if (currentInput.includes('总结') || currentInput.includes('概括')) {
      emit('save', aiContent)
    }
